from data_handlers.loader import  load_weather_from_html
from models.weather import WeatherData

class WeatherDataManager:
    def __init__(self, path):
        self.weather = [WeatherData(**row) for row in load_weather_from_html(path)]

    def for_region(self, region):
        return [w for w in self.weather if w.location_id == region]

    def for_date(self, date):
        return [w for w in self.weather if w.date == date]
